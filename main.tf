...
resource "aws_ami_copy" "copied_ami" {
  ...
  source_ami_region = "ap-south-2"

  depends_on = [aws_ami_launch_permission.share_to_destination]
}

# Sleep for some time to allow the AMI to be available
resource "time_sleep" "wait_for_ami" {
  depends_on      = [aws_ami_copy.copied_ami]
  create_duration = "200s" # Wait for ~3.3 minutes
}

# Create a security group in destination account
...
# Launch EC2 instance in destination using copied AMI
resource "aws_instance" "web_server" {
  provider                    = aws.destination
  ami                         = aws_ami_copy.copied_ami.id
  instance_type               = "t3.micro"
  key_name                    = "newacc"  # Make sure this key exists in destination
